...
import com.androidx.view.tab.utils.FragmentChangeManager;
...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
     ...
     * 关联数据支持同时切换fragments
     */
    public void setTabData(List<CustomTabEntity> tabEntitys, FragmentActivity fa, int containerViewId, ArrayList<Fragment> fragments) {
     ...
     * 关联数据支持同时切换fragments
     */
    public void setTabData(List<CustomTabEntity> tabEntitys, AppCompatActivity a, int containerViewId, ArrayList<Fragment> fragments) {
        mFragmentChangeManager = new FragmentChangeManager(a.getSupportFragmentManager(), containerViewId, fragments);
        setTabData(tabEntitys);
    }
    ...
    private void addTab(final int position, View tabView) {
        ...
        AppCompatImageView iv_tab_icon = tabView.findViewById(R.id.iv_tab_icon);
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIconVisible) {
                ...
                LinearLayoutCompat.LayoutParams lp = new LinearLayoutCompat.LayoutParams(
                        mIconWidth <= 0 ? LinearLayoutCompat.LayoutParams.WRAP_CONTENT : (int) mIconWidth,
                        mIconHeight <= 0 ? LinearLayoutCompat.LayoutParams.WRAP_CONTENT : (int) mIconHeight);
                if (mIconGravity == Gravity.LEFT) {
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public AppCompatImageView getIconView(int tab) {
    ...
    public void setMsgMargin(int position, float leftPadding, float bottomPadding) {
        ...
        if (tipView != null) {
            ...
            if (mIconVisible) {
                if (iconH <= 0) {
                    iconH = mContext.getResources().getDrawable(mTabEntitys.get(position).getTabSelectedIcon(), mContext.getTheme()).getIntrinsicHeight();
                }
                margin = mIconMargin;
            }
